    ...
    public class BaseConvertDataFunctionalTests
    {
        ...
        public static IEnumerable<object[]> GetDataForHl7v2()
        {
            var data = new List<string[]>
            {
                ...
                new[]
                {
                    ...
                    @"LRI_2.0-NG_CBC_Typ_Message-expected.json"
                },
                ...
                new[]
                {
                    ...
                    @"IZ_1_1.1_Admin_Child_Max_Message-expected.json"
                },
        ...
        public static IEnumerable<object[]> GetDataForCcda()
        {
            var data = new List<string[]>
            {
                ...
                {
                    @"CCD",
                    @"CCD-Parent-Document-Replace-C-CDAR2.1.ccda",
                    @"CCD-Parent-Document-Replace-C-CDAR2.1-expected.json"
                },
                new[] { @"ConsultationNote", @"Care_Plan.ccda", @"Care_Plan-expected.json" },
                new[]
                {
                    @"ConsultationNote",
                    @"CDA_with_Embedded_PDF.ccda",
                    @"CDA_with_Embedded_PDF-expected.json"
                },
                new[]
                {
                    @"ConsultationNote",
                    @"Consultation_Note.ccda",
                    @"Consultation_Note-expected.json"
                },
                new[]
                {
                    @"ConsultationNote",
                    @"Unstructured_Document_embed.ccda",
                    @"Unstructured_Document_embed-expected.json"
                },
                new[]
                {
                    @"DischargeSummary",
                    @"Discharge_Summary.ccda",
                    @"Discharge_Summary-expected.json"
                },
                new[]
                {
                    @"DischargeSummary",
                    @"Consult-Document-Closing-Referral-C-CDAR2.1.ccda",
                    @"Consult-Document-Closing-Referral-C-CDAR2.1-expected.json"
                },
                new[]
                {
                    @"HistoryandPhysical",
                    @"History_and_Physical.ccda",
                    @"History_and_Physical-expected.json"
                },
                new[]
                {
                    @"HistoryandPhysical",
                    @"Diagnostic_Imaging_Report.ccda",
                    @"Diagnostic_Imaging_Report-expected.json"
                },
                ...
                {
                    @"ProcedureNote",
                    @"Patient-and-Provider-Organization-Direct-Address-C-CDAR2.1.ccda",
                    @"Patient-and-Provider-Organization-Direct-Address-C-CDAR2.1-expected.json"
                },
                new[] { @"ProgressNote", @"Progress_Note.ccda", @"Progress_Note-expected.json" },
                new[]
                {
                    @"ProgressNote",
                    @"PROBLEMS_in_Empty_C-CDA_2.1-C-CDAR2.1.ccda",
                    @"PROBLEMS_in_Empty_C-CDA_2.1-C-CDAR2.1-expected.json"
                },
                ...
                {
                    @"TransferSummary",
                    @"Transfer_Summary.ccda",
                    @"Transfer_Summary-expected.json"
                },
                new[]
                {
                    @"TransferSummary",
                    @"Unstructured_Document_reference.ccda",
                    @"Unstructured_Document_reference-expected.json"
                },
            };
            return data.Select(item =>
                new[]
        ...
        public static IEnumerable<object[]> GetDataForJson()
        {
            var data = new List<string[]>
            {
                new[]
                {
                    @"ExamplePatient",
                    @"ExamplePatient.json",
                    @"ExamplePatient-expected.json"
                },
                new[]
                {
                    @"Stu3ChargeItem",
                    @"Stu3ChargeItem.json",
                    @"Stu3ChargeItem-expected.json"
                },
            };
            return data.Select(item =>
                new[]
        ...
        public static IEnumerable<object[]> GetDataForFhirToHl7v2()
        {
            var data = new List<string[]>
            {
                new[]
                {
                    ...
                    @"ObservationBundle-expected.hl7"
                },
            };
            return data.Select(item =>
                new[]
        ...
        protected void ConvertHl7v2MessageAndValidateExpectedResponse(
        ...
        )
        {
            ...
            var expectedObject = serializer.Deserialize<JObject>(
                new JsonTextReader(new StringReader(expectedContent))
            );
            var actualObject = serializer.Deserialize<JObject>(
                new JsonTextReader(new StringReader(actualContent))
            );
        ...
        protected void ConvertCCDAMessageAndValidateExpectedResponse(
            ITemplateProvider templateProvider,
            string rootTemplate,
            string inputFile,
            string expectedFile
        )
        {
            var ccdaProcessor = new CcdaProcessor(
                _processorSettings,
                FhirConverterLogging.CreateLogger<CcdaProcessor>()
            );
            var inputContent = File.ReadAllText(inputFile);
            ...
            if (
                expectedObject["entry"]?.Last()["resource"]["resourceType"].ToString()
                == "DocumentReference"
            )
            {
        ...
        protected void ValidateConvertCCDAMessageIsValidFHIR(
            ITemplateProvider templateProvider,
            string rootTemplate,
            string inputFile,
            string validationFailureStep,
            int numFailures
        )
        {
            var validateFhir = Environment.GetEnvironmentVariable("VALIDATE_FHIR") ?? "true";
            if (validateFhir.Trim() == "false")
                return;
            ...
            var inputContent = File.ReadAllText(inputFile);
            var actualContent = ccdaProcessor.Convert(
        ...
        protected void ConvertFHIRMessageAndValidateExpectedResponse(
            ITemplateProvider templateProvider,
            string rootTemplate,
            string inputFile,
            string expectedFile
        )
        {
            var fhirProcessor = new FhirProcessor(
                _processorSettings,
                FhirConverterLogging.CreateLogger<FhirProcessor>()
            );
            var inputContent = File.ReadAllText(inputFile);
            var expectedContent = File.ReadAllText(expectedFile);
        ...
        protected void ConvertJsonMessageAndValidateExpectedResponse(
            ITemplateProvider templateProvider,
            string rootTemplate,
            string inputFile,
            string expectedFile
        )
        {
            var jsonProcessor = new JsonProcessor(
                _processorSettings,
                FhirConverterLogging.CreateLogger<JsonProcessor>()
            );
            var inputContent = File.ReadAllText(inputFile);
            var expectedContent = File.ReadAllText(expectedFile);
        ...
        protected void ConvertFhirBundleToHl7v2AndValidateExpectedResponse(
            ITemplateProvider templateProvider,
            string rootTemplate,
            string inputFile,
            string expectedFile
        ...
        {
            var jsonProcessor = new FhirToHl7v2Processor(
                _processorSettings,
                FhirConverterLogging.CreateLogger<FhirToHl7v2Processor>()
            );
            var inputContent = File.ReadAllText(inputFile);
            var expectedContent = File.ReadAllText(expectedFile);
            var actualContent = jsonProcessor.Convert(inputContent, rootTemplate, templateProvider);
