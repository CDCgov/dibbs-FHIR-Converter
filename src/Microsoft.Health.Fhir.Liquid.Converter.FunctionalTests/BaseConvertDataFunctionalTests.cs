    ...
    public class BaseConvertDataFunctionalTests
    {
        ...
        public static IEnumerable<object[]> GetDataForEcr()
        {
            var data = new List<string[]>
            {
                ...
                // ]
                new[] { @"EICR", @"eCR_full.xml", @"eCR_full-expected.json", "validation", "10" },
                new[] { @"EICR", @"eCR_RR_combined_3_1.xml", @"eCR_RR_combined_3_1-expected.json", "validation", "14" },
                new[] { @"EICR", @"eCR_EveEverywoman.xml", @"eCR_EveEverywoman-expected.json", "validation", "52" },
                new[] { @"EICR", @"eicr04152020.xml", @"eicr04152020-expected.json", "validation", "20" },
                new[] { @"EICR", @"CDAR2_IG_PHCASERPT_R2_D2_SAMPLE.xml", @"CDAR2_IG_PHCASERPT_R2_D2_SAMPLE-expected.json", "validation", "26" },
            };
            return data.Select(item => new[]
            {
                item[0],
