    ...
    public class BaseConvertDataFunctionalTests
    {
        ...
        public static IEnumerable<object[]> GetDataForEcr()
        {
            var data = new List<string[]>
            {
                ...
                new[] { @"EICR", @"eCR_EveEverywoman.xml", @"eCR_EveEverywoman-expected.json", "validation", "48" },
                new[] { @"EICR", @"eicr04152020.xml", @"eicr04152020-expected.json", "validation", "19" },
                new[] { @"EICR", @"CDAR2_IG_PHCASERPT_R2_D2_SAMPLE.xml", @"CDAR2_IG_PHCASERPT_R2_D2_SAMPLE-expected.json", "validation", "23" },
            };
            return data.Select(item => new[]
            {
                item[0],
